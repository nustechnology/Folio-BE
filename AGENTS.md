...
- `Passage.embedding` is `vector(1536)`, matching `gemini-embedding-2` truncated to `MODEL_EMBEDDING_DIMENSIONS`. Changing the model or the width needs BOTH a migration on that column and a full re-index (`yarn reingest`) — vectors from different models are not comparable, and pgvector rejects a mismatched width outright
